  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl sm:text-3xl font-semibold p-4 underline text-blue-900">Trending {category.charAt(0).toUpperCase() + category.slice(1).toLowerCase()}</h1>
          ...
          <div key={product.id} className="border p-4 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 bg-white">
            ...
            <div className="flex gap-8">
              ...
              {product.id in cart ? (
                ...
                <button className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-1 px-4 rounded-lg transition-colors duration-300" onClick={() => handleAddToCart(product)}>
                  Add to Cart
                </button>
              )}
            </div>
